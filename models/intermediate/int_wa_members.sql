{{
    config(
        materialized='table',
        post_hook=[
            "create index if not exists idx_int_wa_members_phone on {{ this }} (normalized_phone)",
            "create index if not exists idx_int_wa_members_placeholder on {{ this }} (placeholder_name)",
        ]
    )
}}
...
duration_calc as (
    ...
    select
        ...
        joined_at,
        left_at,
        coalesce(left_at, current_timestamp) - joined_at as duration_in_group,
        last_event_type,
        status,
        {{ normalize_wa_phone('user_phone') }}           as normalized_phone,
        {{ extract_wa_placeholder_name('user_phone') }}  as placeholder_name
    from members
